...
const VALID_TYPES = new Set(['allergy', 'food', 'supplement', 'pregnancy', 'lactation', 'driving', 'age', 'administration', 'general']);
...
warningsData.forEach((w, i) => {
  ...
  if (!VALID_TYPES.has(w.type)) {
    errors.push(`${where}: geçersiz type '${w.type}' (allergy|food|supplement|pregnancy|lactation|driving|age|administration|general)`);
  }
